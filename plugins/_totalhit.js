...
const handler = async (m, { conn }) => {
  try {
    const stats = _.chain(Object.entries(db.data.stats))
      .map(([key, val]) => {
        // Check if plugins[key] exists and has a 'help' property
        const helpText = plugins[key] && plugins[key].help
          ? Array.isArray(plugins[key].help)
            ? plugins[key].help.join(" , ")
            : plugins[key].help
          : key.split("/").pop().slice(0, -3); // Default fallback
...
const getTime = (ms) => {
  ...
  return (
    `${now.days ? now.days + "d" : ""} ${now.hours ? now.hours + "h" : ""} ${now.minutes ? now.minutes + "m" : ""} ${now.seconds ? now.seconds + "s" : ""} ${now.milliseconds ? now.milliseconds + "ms" : ""}`.trim() ||
    "a few seconds"
  );
};

...
const handler = async (m, { conn, args, usedPrefix, command }) => {
  try {
    ...
    // Send the processing message and react
    m.reply("*PROSES*\n\n> Untuk Meng Extraxt Mungkin, Membutuhkan Waktu Beberapa Saat Harap Tunggu Sebentar");
    m.react('📝');

    const result = await SaveWeb2zip(url);
    if (!result) return m.reply("Terjadi kesalahan saat memproses URL.");

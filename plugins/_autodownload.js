...
export async function before(m) {
    ...
    // Pastikan user limit ada
    if (!user.limit) user.limit = 0;

    // Fungsi untuk memeriksa apakah teks mengandung link media sosial
    const containsLink = (text) => {
        return fbRegex.test(text) || igRegex.test(text) || tiktokRegex.test(text) || twitRegex.test(text);
    };

    if (m.isGroup) {
        if (!chat.autodl) return false; // Jika autodl tidak diaktifkan, keluar

        if (containsLink(m.text) && user.limit <= 0) {
        ...
        try {
            ...
            if (m.text.trim().match(fbRegex)) {
                ...
                await conn.sendMessage(m.chat, { video: await fetchBuffer(urls[0].url), caption: description }, { quoted: m });
            ...
            // TikTok
                ...
                const { download, stats, username } = await slide(tiktokUrl);
                let hasil = Object.values(download).filter(v => v.type == "slide");
                if (hasil.length === 0) {
                    await conn.sendFile(m.chat, download[0].link, '', `username: \`${username}\`\n👀: \`${stats.views}\`\n♥️: \`${stats.likes}\`\n💬: \`${stats.comments}\`\n↪️:  \`${stats.shares}\`\n📥: \`${stats.downloads}\``, m);
                } else {
                    for (let i of hasil) {
            ...
            console.error(e);
            await m.reply("Terjadi kesalahan saat mencoba mendownload media.");
        }
    }
}
...
// --- Functions untuk mendownload media --- 
